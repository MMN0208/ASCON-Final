...
int main() {
	...
	p.data = 0x4141414141414180;
	p.last = 1;
	test_in << p;

	// call
	ascon128(0, 0, 0, 1, test_in, test_itag, test_out, &test_otag, &test_success, &hash);
	...
	ascon128(0, 0, 1, 1, test_in, test_itag, test_out, &test_otag, &test_success, &hash);
	...
	ascon128(0, 0, 2, 0, test_in, test_itag, test_out, &test_otag, &test_success, &hash);

	// read hash;
	std::cout << std::hex << hash << std::endl;
	std::cout << std::hex << test_success << std::endl;
	std::cout << "Test ascon_three_mode: Success" << std::endl;
	return 0;
}
